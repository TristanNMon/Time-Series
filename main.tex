...
\begin{solution}  % ANSWER HERE
1) We fix all the dictionary atoms $\mathbf{d}_k \in \mathbb{R}^{L}$ for $k = 1, \dots, K$,
...
2) We use the Question 1 result with the lasso formulation above with response vector $\mathbf{x}$ and design matrix $\mathbf{D}$ to have:
\[\boxed{
\lambda_{\max} = \| \mathbf{D}^T \mathbf{x} \|_\infty.
}\]
